...
| `edit/new.rs` | creates edited agent or human files; validates human assignments, encrypts stdin with the target-path rule, and scrubs the editor's plaintext temp file; `set_human` reads a one-line value from stdin with core dumps disabled and encrypts it directly to the target without a plaintext temp file |
